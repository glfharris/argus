...
class Bluetooth:
    ...
    async def _notification_handler(self, _: Any, data: bytearray) -> None:
        ...
        if data[0] == FrameDataTypePrefixes.LONG_TEXT.value:
            ...
            if self._print_debugging:
                print(
                    f"Received chunk #{self._ongoing_print_response_chunk_count}: "
                    + data[1:].decode()
                )
            if len(self._ongoing_print_response) > self._max_receive_buffer:
                raise Exception(
                    f"Buffered received long printed string is more than {self._max_receive_buffer} bytes"
                )
            ...
            if len(total_expected_chunk_count_as_string) > 0:
                total_expected_chunk_count: int = int(
                    total_expected_chunk_count_as_string
                )
                if self._print_debugging:
                    print(
                ...
                if (
                    self._ongoing_print_response_chunk_count
                    != total_expected_chunk_count
                ):
                    raise Exception(
                        f"Chunk count mismatch in long received string (expected {total_expected_chunk_count}, got {self._ongoing_print_response_chunk_count})"
                    )
            ...
            if self._print_debugging:
                print(
                    "Finished receiving long printed string: "
            ...
            if len(self._ongoing_data_response) > self._max_receive_buffer:
                raise Exception(
                    f"Buffered received long raw data is more than {self._max_receive_buffer} bytes"
                )
            ...
            if len(total_expected_chunk_count_as_string) > 0:
                ...
                if self._print_debugging:
                    print(
                        f"Received final data chunk count: {total_expected_chunk_count}"
                    )
                if (
                    self._ongoing_data_response_chunk_count
                    != total_expected_chunk_count
                ):
                    raise Exception(
                        f"Chunk count mismatch in long received data (expected {total_expected_chunk_count}, got {self._ongoing_data_response_chunk_count})"
                    )
    ...
    async def connect(
        ...
        devices: Dict[str, Tuple[Any, Any]] = await BleakScanner.discover(
            3, return_adv=True
        )
    ...
    async def _transmit(self, data: bytearray) -> None:
        ...
        if len(data) > self._btle_client.mtu_size - 3:
            raise Exception(
                f"Payload length is too large: {len(data)} > {self._btle_client.mtu_size - 3}"
            )
    ...
    async def wait_for_print(self, timeout: Optional[float] = None) -> str:
        ...
        try:
            ...
            raise Exception(
                f"Frame didn't respond with printed data (from print() or prntLng()) within {timeout} seconds"
            )
    ...
    async def wait_for_data(self, timeout: Optional[float] = None) -> bytes:
        ...
        try:
            ...
            raise Exception(
                f"Frame didn't respond with data (from frame.bluetooth.send(data)) within {timeout} seconds"
            )
    ...
    async def send_data(
        self, data: bytearray, await_data: bool = False
    ) -> Optional[bytes]:
        """
