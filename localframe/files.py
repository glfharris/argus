...
class Files:
    ...
    async def write_file(self, path: str, data: bytes, checked: bool = False) -> None:
        ...
        response = await self.frame.bluetooth.send_lua(
            f'w=frame.file.open("{path}","write")' + (';print("o")' if checked else ""),
            await_print=checked,
        )
        ...
        response = await self.frame.bluetooth.send_lua(
            ...
            + (';print("c")' if checked else ""),
            await_print=checked,
        )
        if checked and response != "c":
            raise Exception(
                f'Couldn\'t register callback for writing to file "{path}": {response}'
            )
        ...
        response = await self.frame.bluetooth.send_lua(
            'w:close();print("c")', await_print=checked
        )
        if checked and response != "c":
    ...
    async def delete_file(self, path: str) -> bool:
        ...
        """
        response = await self.frame.bluetooth.send_lua(
            f'frame.file.remove("{path}");print("d")', await_print=True
        )
        return response == "d"
