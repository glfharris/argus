...
class Display:
    ...
    async def show_text(
        ...
        color: PaletteColors = PaletteColors.WHITE,
    ):
        """
    ...
    async def write_text(
        ...
        if vertical_align == "middle":
            vertical_offset = (
                max_height if max_height is not None else (400 - y)
            ) // 2 - total_height_of_text // 2
        elif vertical_align == "bottom":
            vertical_offset = (
                max_height if max_height is not None else (400 - y)
            ) - total_height_of_text
        ...
        for line in text.split("\n"):
            ...
            if horizontal_align == "center":
                this_line_x = (
                    x
                    + (max_width if max_width is not None else (640 - x)) // 2
                    - self.get_text_width(line) // 2
                )
            elif horizontal_align == "right":
                this_line_x = (
                    x
                    + (max_width if max_width is not None else (640 - x))
                    - self.get_text_width(line)
                )
    ...
    async def scroll_text(
        ...
        if total_height < 400:
            ...
            return
        await self.frame.run_lua(
            f"scrollText(\"{self.frame.escape_lua_string(text)}\",{self.line_height},{total_height},{lines_per_frame},{delay},'{color.name}',{self.char_spacing})",
            checked=True,
            timeout=total_height / lines_per_frame * (delay + 0.1) + 5,
        )
    ...
    async def set_palette(
        self, paletteIndex: PaletteColors, rgb_color: tuple[int, int, int]
    ):
        """
        ...
        await self.frame.run_lua(
            f"frame.display.assign_color({paletteIndex.name},{color[0]},{color[1]},{color[2]})",
            checked=True,
        )
    ...
    async def draw_rect(
        self, x: int, y: int, w: int, h: int, color: PaletteColors = PaletteColors.WHITE
    ):
        """
    ...
    async def draw_rect_filled(
        ...
        lua_to_send += self._draw_rect_lua(
            x + border_width,
            y + border_width,
            w - border_width * 2,
            h - border_width * 2,
