...
function prntLng(stringToPrint)
    ...
    while i <= len do
        ...
        print('\\x"""
    + FrameDataTypePrefixes.LONG_TEXT.value_as_hex
    + """'..chunk)
        chunkIndex = chunkIndex + 1
        i = j + 1
    end
    print('\\x"""
    + FrameDataTypePrefixes.LONG_TEXT_END.value_as_hex
    + """'..chunkIndex)
end
function sendPartial(dataToSend, max_size)
    ...
    while i <= len do
        ...
        while true do
            if pcall(frame.bluetooth.send, '\\x"""
    + FrameDataTypePrefixes.LONG_DATA.value_as_hex
    + """' .. chunk) then
                break
            end
        end
...
function printCompleteFile(filename)
    ...
    while true do
        if pcall(frame.bluetooth.send, '\\x"""
    + FrameDataTypePrefixes.LONG_DATA_END.value_as_hex
    + """' .. chunkIndex) then
            break
        end
    end
...
function cameraCaptureAndSend(quality,autoExpTimeDelay,autofocusType,exposure,exposureSpeed,shutterLimit,analogGainLimit,whiteBalanceSpeed)
    ...
    while true do
        if state == 'EXPOSING' then
            if frame.time.utc() - last_autoexp_time > 0.1 then
                frame.camera.auto { metering = autofocusType,exposure=exposure,exposure_speed=exposureSpeed,shutter_limit=shutterLimit,analog_gain_limit=analogGainLimit,white_balance_speed=whiteBalanceSpeed }
...
function microphoneRecordAndSend(sample_rate, bit_depth, max_time_in_seconds)
    ...
    + """' .. s) then
                    ...
                    if pcall(frame.bluetooth.send, '\\x"""
    + FrameDataTypePrefixes.MIC_DATA.value_as_hex
    + """' .. s) then
                        break
                    end
                end
