...
class Direction:
    ...
    def __str__(self) -> str:
        ...
        """
        return (
            f"Direction(roll={self.roll}, pitch={self.pitch}, heading={self.heading})"
        )
    ...
    def __add__(self, other: Direction) -> Direction:
        ...
        return Direction(roll=new_roll, pitch=new_pitch, heading=new_heading)
...
class Motion:
    ...
    async def get_direction(self) -> Direction:
        ...
        result = await self.frame.run_lua(
            "local dir = frame.imu.direction();print(dir['roll']..','..dir['pitch']..','..dir['heading'])",
            await_print=True,
        )
        result = result.split(",")
        direction = Direction(
            roll=float(result[0]), pitch=float(result[1]), heading=float(result[2])
        )

        return direction

    async def run_on_tap(
        self,
        ...
        if callback is not None:
            self.frame.bluetooth.register_data_response_handler(
                FrameDataTypePrefixes.TAP, callback
            )
        else:
            self.frame.bluetooth.register_data_response_handler(
                FrameDataTypePrefixes.TAP, None
            )

        if lua_script is not None and callback is not None:
            await self.frame.run_lua(
                "function on_tap();frame.bluetooth.send('\\x"
                + FrameDataTypePrefixes.TAP.value_as_hex
                + "');"
                + lua_script
                + ";end;frame.imu.tap_callback(on_tap)",
                checked=True,
            )
