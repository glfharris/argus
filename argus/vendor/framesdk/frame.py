...
class Frame:
    ...
    async def ensure_connected(self) -> None:
        ...
        if not self.bluetooth.is_connected():
            ...
            await self.run_lua(
                f"is_awake=true;frame.time.utc({int(time.time())});frame.time.zone('{time.strftime('%z')[:3]}:{time.strftime('%z')[3:]}')",
                checked=True,
            )
    ...
    async def evaluate(self, lua_expression: str) -> str:
        ...
        """
        await self.ensure_connected()
        return await self.run_lua(
            f"prntLng(tostring({lua_expression}))", await_print=True
        )
    ...
    async def run_lua(
        ...
        if len(lua_string) <= self.bluetooth.max_lua_payload():
            if checked and not await_print:
                lua_string += ';print("+")'
                if len(lua_string) <= self.bluetooth.max_lua_payload():
                    result = await self.bluetooth.send_lua(
                        lua_string, await_print=True, timeout=timeout
                    )
                    if result != "+":
                        raise Exception(f"Lua did not run successfully: {result}")
                    return None
            else:
                return await self.bluetooth.send_lua(
                    lua_string, await_print=await_print, timeout=timeout
                )

        return await self.send_long_lua(
            lua_string, await_print=await_print, checked=checked, timeout=timeout
        )

    async def send_long_lua(
        self,
        string: str,
        await_print: bool = False,
        checked: bool = False,
        ...
        await self.files.write_file(
            f"/{random_name}.lua", string.encode(), checked=True
        )
        if await_print:
            response = await self.bluetooth.send_lua(
                f'require("{random_name}")', await_print=True, timeout=timeout
            )
        elif checked:
            response = await self.bluetooth.send_lua(
                f"require(\"{random_name}\");print('done')",
                await_print=True,
                timeout=timeout,
            )
            if response != "done":
    ...
    async def get_battery_level(self) -> int:
        ...
        await self.ensure_connected()
        response = await self.bluetooth.send_lua(
            "print(frame.battery_level())", await_print=True
        )
            ...
            if self._lua_on_wake is not None or self._callback_on_wake is not None:
                ...
                if self._callback_on_wake is not None:
                    run_on_wake = (
                        ...
                        + FrameDataTypePrefixes.WAKE.value_as_hex
                        + "');"
                        + run_on_wake
                    )
                run_on_wake = (
                    "if not is_awake then;is_awake=true;" + run_on_wake + ";end"
                )
    ...
    async def inject_library_function(
        self, name: str, function: str, version: int
    ) -> None:
        """
        ...
        exists = await self.bluetooth.send_lua(
            f"print({name} ~= nil)", await_print=True
        )
        ...
        if exists != "true":
            ...
            if exists:
                response = await self.bluetooth.send_lua(
                    f'require("lib-{version}/{name}");print("l")', await_print=True
                )
                if response == "l":
                    return
            ...
            await self.files.write_file(
                f"/lib-{version}/{name}.lua", function.encode(), checked=True
            )
    ...
    async def inject_all_library_functions(self) -> None:
        ...
        await self.ensure_connected()
        response = await self.bluetooth.send_lua(
            f'frame.file.mkdir("lib-{library_version}");print("c")', await_print=True
        )
        if response == "c":
            if self.bluetooth._print_debugging:
        ...
        await self.inject_library_function(
            "prntLng", library_print_long, library_version
        )
    ...
    async def run_on_wake(
        ...
        if callback is not None:
            self.bluetooth.register_data_response_handler(
                FrameDataTypePrefixes.WAKE, lambda data: callback()
            )
        else:
            self.bluetooth.register_data_response_handler(
                FrameDataTypePrefixes.WAKE, None
            )
        ...
        if lua_script is not None and callback is not None:
            await self.files.write_file(
                "main.lua",
                (
                    "is_awake=true;frame.bluetooth.send('\\x"
                    + FrameDataTypePrefixes.WAKE.value_as_hex
                    + "');\n"
                    + lua_script
                ).encode(),
            ...
            await self.files.write_file(
                "main.lua", ("is_awake=true;" + lua_script).encode(), checked=True
            )
        else:
