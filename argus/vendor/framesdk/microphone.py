...
class Microphone:
    ...
    async def record_audio(
        self,
        silence_cutoff_length_in_seconds: Optional[int] = 3,
        max_length_in_seconds: int = 30,
    ) -> np.ndarray:
        """
        ...
        self._audio_buffer = np.array(
            [], dtype=np.int8 if self.bit_depth == 8 else np.int16
        )
        self.frame.bluetooth.register_data_response_handler(
            FrameDataTypePrefixes.MIC_DATA, self._audio_buffer_handler
        )
        self._audio_finished_event.clear()

        bytes_per_second = self.sample_rate * (self.bit_depth // 8)
        seconds_per_byte = 1 / bytes_per_second
        self._seconds_per_packet = seconds_per_byte * (
            self.frame.bluetooth.max_data_payload() - 1
        )
        ...
        if self.frame.bluetooth.print_debugging:
            print(
                f"Starting audio recording at {self.sample_rate} Hz, {self.bit_depth}-bit"
            )
        await self.frame.bluetooth.send_lua(
            f"microphoneRecordAndSend({self.sample_rate},{self.bit_depth},nil)"
        )
        try:
            await asyncio.wait_for(
                self._audio_finished_event.wait(), timeout=max_length_in_seconds
            )
            await self.frame.bluetooth.send_break_signal()
            # Trim the final _silence_cutoff_length_in_seconds seconds
            trim_length = (
                self._silence_cutoff_length_in_seconds - 0.5
            ) * self._sample_rate
        ...
        self.frame.bluetooth.register_data_response_handler(
            FrameDataTypePrefixes.MIC_DATA, None
        )
    ...
    async def save_audio_file(
        self,
        filename: str,
        silence_cutoff_length_in_seconds: int = 3,
        max_length_in_seconds: int = 30,
    ) -> float:
        """
        ...
        audio_data = await self.record_audio(
            silence_cutoff_length_in_seconds, max_length_in_seconds
        )
        ...
        scale_factor = np.min(
            [
                ideal_range / real_range,
                int(np.iinfo(np.int16).max) / np.max(audio_data),
                int(np.iinfo(np.int16).min) / np.min(audio_data),
            ]
        )
    ...
    def _audio_buffer_handler(self, data: bytes) -> None:
        ...
        if self._silence_cutoff_length_in_seconds is not None:
            ...
            if delta - self._noise_floor > self._silence_threshold:
                ...
                if (
                    time.time() - self._last_sound_time
                    > self._silence_cutoff_length_in_seconds
                ):
                    self._audio_finished_event.set()
                elif self.frame.bluetooth.print_debugging:
                    print("-", end="", flush=True)

    def _convert_bytes_to_audio_data(
        self, audio_buffer: bytes, bit_depth: int
    ) -> np.ndarray:
        """
    ...
    def play_audio_background(
        ...
        if bit_depth == 8:
            ...
            np.multiply(
                audio_data,
                32767 / np.max(np.abs(audio_data)),
                out=audio_data,
                casting="unsafe",
            )
        else:
        ...
        return simpleaudio.play_buffer(
            audio_data, num_channels=1, bytes_per_sample=2, sample_rate=sample_rate
        )
